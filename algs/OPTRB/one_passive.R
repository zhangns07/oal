...
option_list <- list(make_option(c("-d", "--dataset"), type="character", default='skin',
                    ...
                    make_option(c("-a", "--alg"), type="character", default='onepassive',
...
for (rep in c(1:20)){
    ...
    for (i in seq_len(ntrain)){
        ...
        cum_loss[,k_t] <- cum_loss[,k_t] + loss_func(pred_t,y_t,'logistic')/(M_t) # importance weighted cum_loss
        T_t <- cum_samples[k_t]
        ...
        if (i!= last_i & cum_label != last_cum_label & (i %% checkpoint ==0 || cum_label %% checkpoint == 0)){
            ...
            for(r in c(1:r_per_h)){ opt_Its[r] <- which.min((cum_loss[,r]))}
