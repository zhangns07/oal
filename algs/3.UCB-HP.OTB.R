...
## Update all the non_requesting experts
        ...
        if (i %% 1000 ==0){
            ...
            for(j in shuffle_test[1:ntest]){
                ...
                OTB_curr <- OTB_curr + 
                    loss_func(c(sum(pred_nonreq * OTB_w1) , sum(pred_nonreq * OTB_w2) ,
...
for(i in seq_len(nh)){
    ...
    neg <- sum(as.matrix(X) %*% all_h[i,]  < 0)
    result <- rbind(result, c(err, length(y) - err , neg , length(y) - neg) )
}
...
pos <- sum(as.matrix(X) %*% all_h[i,]  >0)
result <- rbind(result, c(correct, length(y) - correct, pos, length(y) - pos) )
...
c(matrix(rep(seq_len(nh), r_per_h), ncol = r_per_h))[ cum_loss / cum_obs <  0.4]
}
